# [Lightcord](https://github.com/Lightcord/Lightcord) Addons [![Support][support-badge]][support-link] [![PayPal][paypal-badge]][paypal-link] [![Patreon][patreon-badge]][patreon-link]

[support-badge]: https://img.shields.io/badge/Support-%2343b581.svg?style=flat&logo=data:image/png;base64,
[support-link]: https://discord.com/invite/Jx3TjNS

[paypal-badge]: https://img.shields.io/badge/PayPal-%23003087.svg?style=flat&logo=data:image/png;base64,
[paypal-link]: https://paypal.me/MircoWittrien

[patreon-badge]: https://img.shields.io/badge/Patreon-%23F96854.svg?style=flat&logo=data:image/png;base64,
[patreon-link]: https://patreon.com/MircoWittrien


# Lightcord is now dead. Go to the [Lightcord site](https://lightcord.site/) for good alternatives (I suggest goosemod)
## Because of this, I am archiveing this to prevent further breakage.
# I did not make the code here; however, I did edit it so it no longer breaks lightcord.
## Because I did not make the majority of this code, all donation links still point towards DevilBro.
I am back from my break!


A Series of Plugins and Themes for BetterDiscord by DevilBro edited to remove lightcord breakage.

## [Plugins](https://github.com/andmagdo/BetterDiscordAddons/tree/master/Plugins/)
## [Themes](https://github.com/andmagdo/BetterDiscordAddons/tree/master/Themes/)

Copyright (C) 2016-2021 Mirco Wittrien

